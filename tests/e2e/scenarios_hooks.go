...
`
				...
				err := wait.For(func() (bool, error) {
					...
					content, err := os.ReadFile(logFile) //nolint:gosec // G304: test file path
					if err != nil {
						return false, nil
					}
			...
			harness.NewStep("verify run_if=changes respects condition", func(ctx *harness.Context) error {
				...
				skillContent, err := os.ReadFile(skillFile) //nolint:gosec // G304: test file path
				if err != nil {
					return err
				}
				...
				// Read log again
				newContent, err := os.ReadFile(logFile) //nolint:gosec // G304: test file path
				if err != nil {
					return err
				}
