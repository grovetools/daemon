...
func TestRestore_NativeStateSurvives(t *testing.T) {
	...
	if err := os.WriteFile(statePath, stateBytes, 0o644); err != nil { //nolint:gosec // G306: test file
...
func TestRestore_NativeSkipsEnvRegistration(t *testing.T) {
	...
	stateBytes, _ := json.MarshalIndent(stateFile, "", "  ")
	if err := os.WriteFile(filepath.Join(stateDir, "state.json"), stateBytes, 0o644); err != nil { //nolint:gosec // G306: test file
		t.Fatalf("write state.json: %v", err)
	}
...
func TestRemoveStateFile_NoOpWhenAbsent(t *testing.T) {
	...
	statePath := filepath.Join(stateDir, "state.json")
	if err := os.WriteFile(statePath, []byte("{}"), 0o644); err != nil { //nolint:gosec // G306: test file
